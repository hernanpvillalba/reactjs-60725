...
const Checkout = () => {
    ...
    const getOrder = async() =>{
        ...
        if(emailMatch) {
            ...
            Swal.fire({ title:`Muchas gracias por tu compra, tu numero de Orden es ${orderID}`} )
            .then(() =>{
                clearCart()
                navigate('/')
            })
  ...
  return (
    <div>
      <form action="">
        ...
        <Input onChange={updateUser} name='repetirEmail' placeholder='Repetir Email' />
        {
            !emailMatch && (
                <Text color={'red.500'}>El Email no coincide </Text>
            )
        }
